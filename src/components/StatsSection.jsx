  ...
  return (
    <section className=" py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-3  gap-8 text-center">
          {stats.map((item, index) => (
            <div key={index} className="bg-white p-6 rounded-md  hover:shadow-md transition">
               <div className="relative w-full max-w-2xl mx-auto mb-6">
      {/* Trapezoid shape */}
      <div className=" h-16 bg-red-600 clip-trapezoid"></div>
     ...
     {item.count}
              ...
              <h4 className="mt-2 text-lg font-semibold text-gray-800  italic">{item.title}</h4>
              <p className="mt-2 text-sm text-gray-600  italic">{item.description}</p>
            </div>
          ))}
        </div>
      </div>
